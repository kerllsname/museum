export default function initializeDots() {
  ...
  dots.forEach((dot) => {
    dot.addEventListener("click", ({ currentTarget: dot }) =>
      handleDotClick(dot, dots)
    );
  });
}
...
function changeBackgroundImage(dot) {
  ...
  welcome_image.style.backgroundImage =
    "linear-gradient(90deg, rgba(0,0,0,1) 33%, rgba(255,255,255,0) 50%), url('./assets/welcome/" +
    image_name +
    "')";
}
